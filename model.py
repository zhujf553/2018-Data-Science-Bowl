#Modified from https://github.com/matterport/Mask_RCNN
...
def data_augment(image,masks,angle=180):
...
def data_augment2(image,masks,angle=180):
    from skimage import transform
    from skimage.transform import resize
...
def data_augment3(image,masks,angle=180):
    from skimage import transform
    from skimage.transform import resize
    from skimage import color
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    # added others
    if augment:
        image, mask = data_augment2(image, mask)
    '''
    ...
    if augment:
        image, mask, exclude = data_augment3(image, mask)
    ...
    image, mask, class_ids, window, shape = dataset.load_and_augment(config, image_id, augment)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        elif mode == "inference_rois":
            model = KM.Model([input_image, input_image_meta],
                             [rpn_rois],
                             name='mask_rcnn_rois')
        elif mode == "inference_given_rois":
            proposal_count = config.POST_NMS_ROIS_INFERENCE
            input_rois = KL.Input(
                shape=config.IMAGE_SHAPE.tolist(), name="input_rois")
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers, my_callback=None):
        """Train the model.
